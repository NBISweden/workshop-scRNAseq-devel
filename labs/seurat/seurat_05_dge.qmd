...
markers_genes %>%
    group_by(cluster) %>%
    slice_min(p_val_adj, n = 25, with_ties = FALSE) -> top25
head(top25)
```
...
DGE_cell_selection %>%
    group_by(cluster) %>%
    slice_min(p_val, n = 5) -> top5_cell_selection
...
DGE_cell_selection %>%
    group_by(direction) %>%
    slice_min(p_val, n = 20) -> top20_cell_selection
...
DGE_sub %>%
    group_by(direction) %>%
    slice_min(p_val, n = 20) -> top20_sub
