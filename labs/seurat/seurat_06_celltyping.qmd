...
Now plot how many cells of each cell types can be found in each cluster.
...
This dataset has predictions at 3 different levels of annotation wiht `l1` being the more broad cell types and `l3` more detailed annotation.
...
reference_markers %>%
    group_by(cluster) %>%
    slice_min(p_val, n = 100) %>%
    slice_max(avg_log2FC, n = 50) -> top50_cell_selection
