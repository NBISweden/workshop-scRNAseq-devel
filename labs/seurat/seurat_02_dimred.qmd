...
reticulate::use_condaenv("~/miniconda3/envs/seurat5", required = TRUE)
...
A feature of UMAP is that it is not limited by the number of dimensions the data can be reduced into (unlike tSNE). We can simply reduce the dimensions altering the `n.components` parameter. So here we will create a UMAP with 10 dimensions.
...
wrap_plots(
    ...
    DimPlot(alldata, reduction = "UMAP10_on_PCA", group.by = "orig.ident", dims = 1:2, shuffle = TRUE) + 
      ggplot2::ggtitle(label = "UMAP10_on_PCA"),
    DimPlot(alldata, reduction = "UMAP10_on_PCA", group.by = "orig.ident", dims = 3:4, shuffle = TRUE) +
      ggplot2::ggtitle(label = "UMAP10_on_PCA"),
    ncol = 3
) + plot_layout(guides = "collect")
```
