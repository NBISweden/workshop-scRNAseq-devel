...
#| eval: false
# Do not run now!
total_counts_per_cell <- colSums(alldata@assays$RNA@layers$counts)
mito_genes <- rownames(alldata)[grep("^MT-", rownames(alldata))]
alldata$percent_mito2 <- 
  colSums(alldata@assays$RNA@layers$counts[match(mito_genes, Features(alldata@assays$RNA)), ]) /
...
Then we run doubletFinder, selecting the first 10 PCs and a `pK` value of 0.09. To optimize the parameters, you can run the `paramSweep` function in the package.
